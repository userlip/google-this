...
async function search(query, options = {}) {
    ...
    const use_mobile_ua = Reflect.has(options, "use_mobile_ua")
        ? options.use_mobile_ua
        : true;
    ...
    if (typeof query === "object" && ris) {
        ...
        if (ris)
            throw new Utils.SearchError(
                "Reverse image search by URL has been deprecated by Google. Please use a file instead."
            );

        const baseUrl = ris
            ? `${Constants.URLS.W_GOOGLE}searchbyimage`
            : `${Constants.URLS.GOOGLE}search`;
    ...
    if (!(response instanceof Error)) {
        // Convert the response data to a UTF-8 string
        response.data = Buffer.from(response.data).toString("utf-8");
    }
    ...
    const $ = Cheerio.load(
        Utils.refineData(response.data, parse_ads, use_mobile_ua)
    );
    ...
    results.html = Buffer.from(response.data, "utf-8").toString("base64");
...
async function uploadImage(buffer, axios_config) {
    ...
    const response = await Axios.post(
        `${Constants.URLS.GIS}searchbyimage/upload`,
        form_data,
        {
            headers: {
...
async function image(query, options = {}) {
    ...
    const response = await Axios.post(
        `${Constants.URLS.W_GOOGLE}_/VisualFrontendUi/data/batchexecute`,
        form_data,
        {
            params: {
                rpcids: "HoAMBc",
    ...
    const res =
        "[null" +
        (Utils.getStringBetweenStrings(response.data, '"[null', ']"') || "") +
        "]";
    const data = JSON.parse(res.replace(/\\"/g, '"').replace(/\\\\"/g, "'"));

    if (data.length <= 1)
        throw new Utils.SearchError(
            "Got unexpected response from BatchExecute API",
            data
        );

    if (!data[56]?.[1])
        throw new Utils.SearchError(
            data[53]?.[1] || "Unexpected response structure",
            data[53]?.[2] || data
        );
...
async function getTopNews(language = "en", region = "US") {
    const url =
        Constants.URLS.GOOGLE_NEWS +
        `topstories?tab=in&hl=${language.toLocaleLowerCase()}-${region.toLocaleUpperCase()}&gl=${region.toLocaleUpperCase()}&ceid=${region.toLocaleUpperCase()}:${language.toLocaleLowerCase()}`;
    ...
    if (response instanceof Error)
        throw new Error("Could not retrieve top news: " + response.message);
    ...
    const headline_stories_publishers = $(Constants.SELECTORS.PUBLISHER)
        .map((i, el) => $(el).text())
        .get();
    const headline_stories_imgs = $(Constants.SELECTORS.STORY_IMG)
        .map((i, el) => $(el).attr("src"))
        .get();
    const headline_stories_time = $(Constants.SELECTORS.STORY_TIME)
        .map((i, el) => $(el).text())
        .get();
    ...
    $(Constants.SELECTORS.STORY_TITLE).each((i, el) => {
        ...
        results.headline_stories.push({
            ...
            url: `${Constants.URLS.GOOGLE_NEWS}${headline_stories_url.slice(
                2
            )}`,
