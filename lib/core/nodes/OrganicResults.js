  ...
  static parse($, parse_ads = false, is_mobile = true) {
    ...
    const titles = $(Constants.SELECTORS.TITLE)
      .map((_i, el) => {
        ...
        if (!parse_ads && is_ad) return null;

        return is_mobile
          ? $(el).text().trim()
          : $(el).find("h3").text().trim() ||
              $(el).find("a > div > span").first().text().trim();
      })
      .get();
    ...
    const descriptions = $(Constants.SELECTORS.DESCRIPTION)
      .map((_i, el) => {
        const is_ad =
          Reflect.has(
            el.parent?.parent?.parent?.attribs || {},
            "data-text-ad"
          ) ||
    ...
    const urls = $(
      is_mobile ? Constants.SELECTORS.URL : `${Constants.SELECTORS.TITLE} > a`
    )
      .map((_i, el) => {
        const is_ad =
          !!$(el).attr("data-rw") ||
          el.parent?.parent?.attribs?.class?.startsWith("vdQmEd");

        if (!parse_ads && is_ad) return null;

        return $(el).attr("href").replace("/url?q=", "").split("&sa=")[0];
      })
      .get();
    ...
    for (let i = 0; i < titles.length; i++) {
      ...
      const high_res_favicon = `${Constants.URLS.FAVICONKIT}/${
        new URL(url || Constants.URLS.W_GOOGLE).hostname
      }/192`;
      const low_res_favicon = `${
        Constants.URLS.W_GOOGLE
      }s2/favicons?sz=64&domain_url=${
        new URL(url || Constants.URLS.W_GOOGLE).hostname
      }`;
